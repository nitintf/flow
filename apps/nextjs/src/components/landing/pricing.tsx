...
export const Pricing = () => {
  return (
    <section id="pricing" className="relative z-10 overflow-hidden pb-12 pt-20 lg:pb-[90px] lg:pt-[120px]">
      <div className="container mx-auto">
        <div className="-mx-4 flex flex-wrap">
          <div className="w-full px-4">
            <div className="mx-auto mb-[60px] text-center">
              <h2 className="text-h3 font-[700] pb-xxs text-white">Start your journey today</h2>
  ...
  return (
    <>
      <div className="w-full md:w-1/2 lg:w-1/3">
        <div className={cn("relative z-10 mb-10 rounded-[10px] shadow-pricing p-xs bg-background", {
          "outline-btn": active
        })}>
          ...
          <span className={cn("block text-lg font-semibold text-white")}>
            {type} {active && <Badge className="ml-xxs text-white" variant={'outline'}>Most Popular</Badge>}
          </span>
          ...
          <p className={cn("mb-4 border-b border-stroke border-gray-300 pb-4 text-base text-gray-400 mt-2")}>
            {description}
          </p>
...
const List = ({ options, active }: { options: string[], active?: boolean }) => {
  return options.map((option) => (
    <div key={option} className={cn("flex items-center gap-1")}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="white" className={cn("flex-shrink-0 w-4 h-4")}>
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
      </svg>
      <p className={cn("text-mini text-gray-400 dark:text-dark-6")}>{option}</p>
    </div>)
  )
};
